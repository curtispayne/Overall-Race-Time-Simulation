...
def calc(s1,s2,s3,data):
    ...
    results = []
    #Initial loop for tyre choice in first stint
    if s1 == 'hard':
        lap1 = hard_init
        deg1 = hard_deg
        tyre1 = 'hard'
    ...
    for pit_lap1 in range(1,laps):
        #Iterating through second stop on each lap
        for pit_lap2 in range(pit_lap1+1,laps+1):
            stint_1 = 0
            stint_2 = 0
            stint_3 = 0
            ...
            for lap in range (pit_lap1+1,pit_lap2+1):
                stint_lap_num = lap - pit_lap1
                cur_lap = lap2 + (stint_lap_num-1)*deg2
                stint_2 = stint_2 + cur_lap
            #Calculating each laptime in third stint
            for lap in range(pit_lap2+1,laps+1):
                stint_lap_num = lap - pit_lap2
                cur_lap = lap3 + (stint_lap_num-1)*deg3
                stint_3 = stint_3 + cur_lap
            #Calculating total race time for this pit lap
            tot_time = stint_1 + stint_2 + stint_3 + 2*pit_time + lap1_add
            #Ensuring two sets of tyres are used
            if tyre1 == tyre2 == tyre3:
                tot_time = 100000
            #Adding results
            results.append({"Time": tot_time,"Pit Lap 1": pit_lap1,"Pit Lap 2": pit_lap2,"Tyre 1": tyre1,"Tyre 2": tyre2,"Tyre 3": tyre3})
